...
OBSIDIAN_DIR="/Users/longfeihao/workspace/obsidian/Note"
CONTENT_DIR="$QUARTZ_DIR/content"

echo "🍃 Obsidian → Quartz 同步服务启动"
echo "   笔记目录: $OBSIDIAN_DIR"
echo "   Quartz: $QUARTZ_DIR"

# 使用 fswatch 监听变化（使用完整路径）
/opt/homebrew/bin/fswatch -o --recursive --include "\.md$" --exclude "\.obsidian" "$OBSIDIAN_DIR" | while read -r event; do
    echo "📝 检测到变化: $(date '+%Y-%m-%d %H:%M:%S')"
    
    cd "$QUARTZ_DIR"
    
    # 同步 Obsidian 内容到 content 目录
    echo "   同步内容..."
    rsync -av --delete --exclude='.obsidian' --exclude='*.tmp' "$OBSIDIAN_DIR/" "$CONTENT_DIR/"
